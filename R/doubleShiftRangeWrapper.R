...
double.shift.range <- function(y,a,z,x,delta1,delta2,Y.est,A.est,Z.est,
  ...
  for(i in 1:nfolds){
    ...
    dat.plus = lapply(delta1, function(d) as.data.frame(cbind(z+d,x)))
    dat.min = lapply(delta2, function(d) as.data.frame(cbind(z+d,x)))
    ...
    for(jj in 1:j){
      ...
      mp = mu.hat.plus[[jj]]
      mm = mu.hat.min[[jj]]
      lp = lambda.hat.plus[[jj]]
      lm = lambda.hat.min[[jj]]
      pp = pi1[[jj]]
      pm = pi2[[jj]]
      
      pos.cond = ((z[test]+d1) < zmax)
      min.cond = ((z[test]+d2) > zmin)
      ...
      xi1.y.delta = ((y[test] - mu.hat)*(pp/pi.hat) + mp)*pos.cond
      xi1.a.delta = ((a[test] - lambda.hat)*(pp/pi.hat) + lp)*pos.cond
      xi2.y.delta = ((y[test] - mu.hat)*(pm/pi.hat) + mm)*pos.cond
      xi2.a.delta = ((a[test] - lambda.hat)*(pm/pi.hat) + lm)*pos.cond
      ...
      ifvals[s==i,jj] = (xi.y.delta - psihat[i,jj]*(xi.a.delta))/mean(lp-lm, na.rm = T)
